...
**How to apply:** When diagnosing Railway build failures, inspect the actual builder and install command in the build log. Keep the Railway builder configuration explicit, use the Nixpkgs attribute `nodejs_20` (not Replit's `nodejs-20_x` spelling), set `NPM_CONFIG_PRODUCTION=false` for the build phase, pin Node/npm in package metadata, and prefer npm ci with the committed lockfile.