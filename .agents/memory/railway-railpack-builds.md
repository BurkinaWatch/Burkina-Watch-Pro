...
**How to apply:** Keep Node 22/npm 10.9.x, put build-time tools in production dependencies, keep test-only tools in devDependencies, and deploy with `npm ci --omit=dev --ignore-scripts --no-audit --no-fund`.